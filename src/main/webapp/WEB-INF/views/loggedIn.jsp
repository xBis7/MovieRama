...
<html>
	<head>
		...
		<h>MovieRama</h>
		<p>Welcome back <a href="userMovies/${currentUserId}" >${username}</a></p>
		<div id="userActions"><button id="logoutButton" type="logoutButton"><a href="logout" >Logout</a></button></div>
	</head>
	<body>
		<div id="sort">Sort by: <a href="sortByLikes">Likes</a> | <a href="sortByHates">Hates</a> | <a href="sortByDate">Date</a></div>
		<p>
		...
		<c:forEach items="${listOfMovies}" var="movie">
			...
			<div id="movie">
				<h1>${movie.title}</h1>
				<p>Posted by 
				<c:if test = "${username != movie.user.username}" >
				...
				<c:choose>
					<c:when test = "${username == movie.user.username}">
					...
					<c:otherwise>	
							...
							<c:if test="${!empty listOfVotes}">
								<c:forEach items="${listOfVotes}" var="vote">
									<c:if test="${username == vote.user.username && movie.movieId == vote.movie.movieId}">
